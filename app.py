...
# 1. ฟังก์ชันดึงข้อมูลจาก Google Sheet มาเป็น Pandas DataFrame
...
# --- ฟังก์ชันคำนวณและดึงรายชื่อกลุ่มแจ้งเตือน (แก้ไขบั๊กตัด String วันที่) ---
def get_alert_members_list(today_date):
    df_all = load_data_from_sheet("members")
    alert_data = []
    
    if not df_all.empty:
        df_active = df_all[df_all["is_deleted"] == 0]
        for _, row in df_active.iterrows():
            # 🛠️ แก้ไขตรงนี้: ตัดเอาเฉพาะ 10 ตัวแรก (YYYY-MM-DD) เผื่อมีเวลาพ่วงท้ายมา
            raw_date = str(row["expiry_date"])[:10] 
            try:
                exp_date = datetime.datetime.strptime(raw_date, "%Y-%m-%d").date()
            except ValueError:
                continue # หากข้อมูลไม่สมบูรณ์จริงๆ ให้ข้ามแถวนั้นไปก่อน
            ...
            if row["package_type"] == "รายครั้ง":
                if p_left == 0 and g_left == 0:
                    reason.append("🔴 โควตาทุกคลาสหมดเกลี้ยง (0 ครั้ง)")
                    status = "🚨 ขาดต่ออายุ"
                elif (1 <= p_left <= 2) or (1 <= g_left <= 2):
                    reason.append(f"🎟️ สิทธิ์ใกล้หมด (เดี่ยว {p_left}, กลุ่ม {g_left})")
                    status = "⚠️ ควรแจ้งเตือน"
            ...
            if status != "ปกติ":
                alert_data.append({
                    ...
                    "expiry_date": raw_date, # ใช้แบบจัดฟอร์แมตแล้ว
                    "status": status,
...
if choice == "👥 จัดการสมาชิก & สมัครแพ็กเกจ":
        ...
        if submit_member:
            if not name.strip() or not phone.strip():
                ...
                payload = {
                    ...
                    "sheet_name": "members",
                    "row_data": [next_id, name.strip(), phone.strip(), package_type, actual_private, actual_group, calculated_expiry.strftime("%Y-%m-%d"), 0, 0, today_date.strftime("%Y-%m-%d")]
                }
    ...
    with col2:
        ...
        if not df_members.empty:
            ...
            if not df_active.empty:
                df_disp = df_active.rename(columns={
                    "member_id": "รหัส", "name": "ชื่อ", "phone": "เบอร์โทร", "package_type": "ประเภทแพ็กเกจ", "expiry_date": "วันหมดอายุ"
                })
                # ปรับการแสดงผลของวันหมดอายุให้สวยงาม ไม่ติดตัวอักษรเวลา
                ...
                st.dataframe(df_disp[["รหัส", "ชื่อ", "เบอร์โทร", "ประเภทแพ็กเกจ", "คลาสเดี่ยวเหลือ", "คลาสกลุ่มเหลือ", "วันหมดอายุ"]], use_container_width=True, hide_index=True)
            else:
        ...
        st.markdown("---")
        st.subheader("⚙️ เครื่องมือปรับเปลี่ยนสิทธิ์ / ต่ออายุสมาชิกด่วน")
        
        if not df_members.empty:
            df_active = df_members[df_members["is_deleted"] == 0]
            m_select_options = {
                f"ID {r['member_id']}: คุณ {r['name']} ({r['package_type']} | วันหมดอายุเดิม: {str(r['expiry_date'])[:10]})": r 
            ...
            if m_select_options:
                ...
                # 🛠️ แก้ไขจุดนี้ให้ปลอดภัยต่อ ISO string format
                clean_exp_str = str(m_data["expiry_date"])[:10]
                current_exp = datetime.datetime.strptime(clean_exp_str, "%Y-%m-%d").date()
                st.markdown(f"**กำลังจัดการสิทธิ์ของ:** `คุณ {m_data['name']}`")
                
                adj_col1, adj_col2, adj_col3 = st.columns(3)
                
                with adj_col1:
                    st.markdown("🥊 **จำนวนคลาสเดี่ยว (Private)**")
                    if m_data["package_type"] == "รายปี":
                        st.caption("แพ็กเกจรายปี: สิทธิ์เป็นแบบไม่จำกัด")
                        p_action = "ไม่มีผล"
                        p_amt = 0
                    else:
                        st.write(f"ปัจจุบันเหลือ: `{m_data['remaining_private']} ครั้ง`")
                ...
                with adj_col2:
                    st.markdown("👥 **จำนวนคลาสกลุ่ม (Group)**")
                    if m_data["package_type"] == "รายปี":
                        st.caption("แพ็กเกจรายปี: สิทธิ์เป็นแบบไม่จำกัด")
                        g_action = "ไม่มีผล"
                        g_amt = 0
                    else:
                        st.write(f"ปัจจุบันเหลือ: `{m_data['remaining_group']} ครั้ง`")
                ...
                with adj_col3:
                    st.markdown("📅 **ขยายเวลา / แก้ไขวันหมดอายุ**")
                    st.write(f"วันหมดอายุปัจจุบัน: `{clean_exp_str}`")
...
# ==========================================
    ...
    with st.expander("➕ คลิกเพื่อเปิดฟอร์ม เพิ่มคลาสเรียนใหม่ / ระบบ Routine", expanded=False):
        ...
        with col_f1:
            ...
            class_type = st.selectbox("ประเภทคลาส *", ["คลาสเดี่ยว (Private)", "คลาสกลุ่ม (Group)"])
    ...
    if not df_classes.empty:
        # ปลอดภัยจากเวลารูปแบบอื่นโดยตัด 10 ตัวแรกเช่นกัน
                    ...
                    for c_id, c_name, inst, c_type, c_color in class_dict[day]:
                        ...
                        if b_count == 0:
                            if cols_week[d_idx].button("🗑️ ลบ", key=f"del_m_{c_id}"):
                                ...
                                if send_action_to_script(payload): st.rerun()
...
# ==========================================
    ...
    df_members = load_data_from_sheet("members")
    if df_members.empty or len(df_members[df_members["is_deleted"] == 0]) == 0:
        st.warning("⚠️ จำเป็นต้องมีข้อมูลสมาชิกในระบบก่อน")
    else:
        col1, col2 = st.columns(2)
        df_active_members = df_members[df_members["is_deleted"] == 0]

        with col1:
            st.subheader("1. เลือกสมาชิกที่จะเข้าเรียน")
            # แก้ไขป้ายกำกับให้อ่านวันที่สะอาดขึ้นแบบไม่มีส่วนเวลาพ่วงท้าย
            m_options = {
                f"{r['name']} (เดี่ยวเหลือ: {r['remaining_private'] if int(r['remaining_private']) != 99999 else 'ไม่จำกัด'}, กลุ่มเหลือ: {r['remaining_group'] if int(r['remaining_group']) != 99999 else 'ไม่จำกัด'} | หมดอายุ {str(r['expiry_date'])[:10]})": r
        ...
        if class_data is not None:
            ...
            is_private_class = (class_data["class_type"] == "คลาสเดี่ยว (Private)")
            
            slots_left = int(member_data["remaining_private"]) if is_private_class else int(member_data["remaining_group"])
            ...
            if already_checked_in:
                ...
                if st.button("🚀 ยืนยันการเข้าคลาสและตัดสิทธิ์", type="primary"):
                    ...
                    if member_data["package_type"] == "รายครั้ง":
                        if is_private_class: new_p = max(0, new_p - 1)
...
# ==========================================
    ...
    if df_attendance.empty or df_members.empty or df_classes.empty:
        ...
        for _, row in df_merged.iterrows():
            ...
            # ล้างหน้าตาวันที่ให้แสดงแค่ 10 ตัวแรก
    ...
    if not alert_list:
        ...
        if not danger_items:
            ...
            for item in danger_items:
                ...
                with c_col2:
                    st.markdown(f"เดี่ยวเหลือ: `{item['p_left']}` | กลุ่มเหลือ: `{item['g_left']}`")
